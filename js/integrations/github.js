...
async function fetchGithubPRs(repoId) {
    ...
    const apiUrl = `${baseUrl}/repos/${repo.owner}/${repo.repo}/pulls?state=open&base=${encodeURIComponent(repo.branch)}&per_page=100`;
...
async function fetchGithubActions(repoId) {
    ...
    if (!repo) return;
    const baseUrl = repo.url || 'https://api.github.com';
    const apiUrl = `${baseUrl}/repos/${repo.owner}/${repo.repo}/actions/runs?branch=${encodeURIComponent(repo.branch)}&per_page=20`;
    const headers = { Accept: 'application/vnd.github.v3+json' };
    if (repo.token) headers['Authorization'] = `Bearer ${repo.token}`;
    try {
        const resp = await fetch(apiUrl, { headers });
...
async function fetchGithubCommits(repoId) {
    ...
    const apiUrl = `${baseUrl}/repos/${repo.owner}/${repo.repo}/commits?sha=${encodeURIComponent(repo.branch)}&since=${since}&per_page=100`;
    const headers = { Accept: 'application/vnd.github.v3+json' };
    if (repo.token) headers['Authorization'] = `Bearer ${repo.token}`;
    try {
        const resp = await fetch(apiUrl, { headers });
